...
class Trainer(BaseTrainer):
    ...
    def __init__(self, config):
        ...
        if config["freeze"]["enable"]:
            self.freeze = FreezeLayers(
                self.model, config["freeze"]["layers"], config["freeze"]["epochs"]
            )
        else:
            self.freeze = None
        ...
        self.log_step = (
            len(self.datamanager.get_dataloader("train")) // 5,
            len(self.datamanager.get_dataloader("val")) // 5,
        )
        self.log_step = (
            self.log_step[0] if self.log_step[0] > 0 else 1,
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            self.writer.add_scalar(
                "lr", self.optimizer.param_groups[-1]["lr"], global_step=epoch
            )
        ...
        for metric in self.lst_metrics:
            if save_best_metrics[metric]:
                filename = os.path.join(
                    self.checkpoint_dir, "model_best_{}.pth".format(metric)
                )
                self.logger.info(
    ...
    def _print_config(
        ...
        self.logger.info(
            "Model: %s " % (self.config["model"]["name"]) + __prams_to_str(params_model)
        )
        if freeze_layers:
            self.logger.info(
                "Freeze layer: %s, at first epoch %d"
                % (
        ...
        self.logger.info(
            "Loss: %s " % (self.config["loss"]["name"]) + __prams_to_str(params_loss)
        )
        self.logger.info(
            "Optimizer: %s " % (self.config["optimizer"]["name"])
            + __prams_to_str(params_optimizers)
        )
        if params_lr_scheduler != None:
            self.logger.info(
                "Lr scheduler: %s " % (self.config["lr_scheduler"]["name"])
                + __prams_to_str(params_lr_scheduler)
            )
        if clip_grad_norm_:
            self.logger.info(
                "clip_grad_norm_, max_norm: %f"
