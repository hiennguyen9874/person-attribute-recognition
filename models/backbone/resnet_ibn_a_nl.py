...
class NonLocalBlock(nn.Module):
    ...
    def __init__(self, in_channel, reduction_ratio=2):
        ...
        assert (
            in_channel % reduction_ratio == 0
        ), "in_channel must divide by reduction_ratio"
        self.in_channel = in_channel
        self.hidden_channel = in_channel // reduction_ratio

        self.theta = nn.Conv2d(
            self.in_channel, self.hidden_channel, kernel_size=1, stride=1, padding=0
        )
        self.phi = nn.Conv2d(
            self.in_channel, self.hidden_channel, kernel_size=1, stride=1, padding=0
        )
        ...
        self.W = nn.Sequential(
            nn.Conv2d(
                self.hidden_channel, self.in_channel, kernel_size=1, stride=1, padding=0
            ),
...
class ResNet_IBN(nn.Module):
    def __init__(self, block, layers, non_layers, ibn_cfg=("a", "a", "a", None)):
...
def resnet50_ibn_a_nl(pretrained=False, progress=True, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(
            load_state_dict_from_url(model_urls["resnet50_ibn_a"], progress=progress),
            strict=False,
        )
    return model
...
def resnet101_ibn_a_nl(pretrained=False, progress=True, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(
            load_state_dict_from_url(model_urls["resnet101_ibn_a"], progress=progress),
            strict=False,
        )
    return model
