...
class PPE(BaseDataSource):
    ...
    def __init__(
        ...
        for phase in ["train", "val", "test"]:
            ...
            self.weight[phase] = np.divide(
                self.weight[phase], int(len(self.data[phase]))
            )
    ...
    def _processes_dir(self, data_dir):
        ...
        with open(os.path.join(data_dir, "detection_ppe.txt"), "r") as f:
            ...
            for line in f:
                ...
                while j < len(splited_line):
                    ...
                    dict_attribute[splited_line[j]] = list(
                        map(float, splited_line[j + 1 : j + 5])
                    )
                    j += 5
        ...
        for key, value in all_data.items():
            for _sampler in value:
                ...
                if (
                    attribute_label["hard_hat"] == 0
                    and attribute_label["none_hard_hat"] == 0
                ):
                    attribute_label["no_hat"] = 1
                if (
                    attribute_label["safety_vest"] == 0
                    and attribute_label["none_safety_vest"] == 0
                ):
                    attribute_label["none_safety_vest"] = 1
                data[self.map_folder[key]].append(
                    (
                        _sampler[0],
                        np.array(list(attribute_label.values())).astype(np.float32),
                    )
                )
        return data
    ...
    def _exists(self, extract_dir):
        if (
            os.path.exists(os.path.join(extract_dir, "ppe", "ppe"))
