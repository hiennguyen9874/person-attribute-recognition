...
class Wider(BaseDataSource):
    ...
    url = {"Wider-data.zip": "1Unt1qudE6wZdfOmZP3ljqThLhQYvprnD"}
    file_path = {
    ...
    def __init__(
        ...
        super(Wider, self).__init__(
            root_dir, dataset_dir, file_name, image_size=(256, 256)
        )
        if download:
        ...
        if os.path.exists(
            os.path.join(self.root_dir, self.dataset_dir, "processed", "Wider-data")
        ):
            self.data_dir = os.path.join(self.data_dir, "Wider-data")

        self.data = dict()
        self.weight = dict()
        self.data["train"], attribute_train, self.weight["train"] = self._processes_dir(
            os.path.join(self.data_dir, "wiger_attribute_train.json"), self.data_dir
        )
        self.data["val"], attribute_val, self.weight["val"] = self._processes_dir(
            os.path.join(self.data_dir, "wiger_attribute_val.json"), self.data_dir
        )
        self.data["test"], attribute_test, self.weight["test"] = self._processes_dir(
            os.path.join(self.data_dir, "wiger_attribute_test.json"), self.data_dir
        )

        assert neq(len(attribute_train), len(attribute_val), len(attribute_test))
        for i in range(len(attribute_train)):
            if not neq(
                list(attribute_train.keys())[i],
                list(attribute_val.keys())[i],
                list(attribute_test.keys())[i],
            ):
                raise ValueError
            if not neq(
                list(attribute_train.values())[i],
                list(attribute_val.values())[i],
                list(attribute_test.values())[i],
            ):
                raise ValueError
    ...
    def _exists(self, extract_dir):
        if (
            os.path.exists(os.path.join(extract_dir, "Wider-data", "train"))
            and os.path.exists(os.path.join(extract_dir, "Wider-data", "val"))
            and os.path.exists(os.path.join(extract_dir, "Wider-data", "test"))
            and os.path.exists(
                os.path.join(extract_dir, "Wider-data", "wiger_attribute_train.json")
            )
            and os.path.exists(
                os.path.join(extract_dir, "Wider-data", "wiger_attribute_val.json")
            )
            and os.path.exists(
                os.path.join(extract_dir, "Wider-data", "wiger_attribute_test.json")
            )
        ):
            return True
        return False
