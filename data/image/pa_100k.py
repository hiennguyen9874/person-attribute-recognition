...
class PA_100K(BaseDataSource):
    ...
    def __init__(self, root_dir="datasets"):
        ...
        image_name["train"] = [
            os.path.join(data_dir, "images", f["train_images_name"][i][0][0])
            for i in range(80000)
        ]
        label["train"] = f["train_label"][:, np.array(self.group_order)].astype(
            np.float32
        )

        image_name["val"] = [
            os.path.join(data_dir, "images", f["val_images_name"][i][0][0])
            for i in range(10000)
        ]
        label["val"] = f["val_label"][:, np.array(self.group_order)].astype(np.float32)

        image_name["test"] = [
            os.path.join(data_dir, "images", f["test_images_name"][i][0][0])
            for i in range(10000)
        ]
        label["test"] = f["test_label"][:, np.array(self.group_order)].astype(
            np.float32
        )
