...
class RandomBalanceBatchSamplerAttribute(torch.utils.data.Sampler):
    ...
    def __init__(
        ...
        **kwargs
        ...
        assert num_attribute <= len(
            attribute_name
        ), "num of attribute in one batch must less than num of attribute in dataset"
    ...
    def __iter__(self):
        ...
        for _ in range(self.num_iterator):
            idx_selected_attribute = np.random.choice(
                len(self.attribute_name), size=self.num_attribute, replace=True
            )
            batch = []
            for idx in idx_selected_attribute:
                index, attribute = self.attribute_name[idx]
                pos_idxs = np.random.choice(
                    self.pos_dict[attribute], size=self.num_positive, replace=True
                )
                neg_idxs = np.random.choice(
                    self.neg_dict[attribute], size=self.num_negative, replace=True
                )
    ...
    def __iter__(self):
        if self.shuffle:
            ...
            for _, attribute in self.attribute_name:
                ...
                np.random.shuffle(self.neg_dict[attribute])
        for _ in range(self.num_iterator):
            idx_selected_attribute = torch.multinomial(
                self.weight, self.num_attribute, replacement=True
            )
            batch = []
            for idx in idx_selected_attribute:
                index, attribute = self.attribute_name[idx]
...
class RandomBatchSamplerAttributeWeight(torch.utils.data.Sampler):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.num_positive = np.rint(num_sampler * weight1 / (weight1 + weight2)).astype(
            int
        )
    ...
    def __iter__(self):
        ...
        for _ in range(self.num_iterator):
            ...
            for idx in idx_selected_attribute:
                index, attribute = self.attribute_name[idx]
                pos_idxs = np.random.choice(
                    self.pos_dict[attribute], size=self.num_positive[idx], replace=True
                )
                neg_idxs = np.random.choice(
                    self.neg_dict[attribute], size=self.num_negative[idx], replace=True
                )
                batch.extend(list(zip(pos_idxs, repeat(index))))
                batch.extend(list(zip(neg_idxs, repeat(index))))
            yield batch
...
class RandomIdentitySampler(torch.utils.data.Sampler):
    ...
    def __init__(
        self, datasource, batch_size=1, num_instances=1, index_dict=None, **kwargs
    ):
        self.datasource = datasource
    ...
    def __iter__(self):
        ...
        while len(avai_pids) >= self.num_pids_per_batch:
            idx_selected_pids = np.random.choice(
...
class RandomBalanceBatchSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self):
        for _ in range(self.num_iterators):
            idx_selected_pids = np.random.choice(
                len(self.person_ids), size=self.num_pids_per_batch, replace=True
            )
            batch = []
            for idx in idx_selected_pids:
                idxs = np.random.choice(
                    self.index_dict[self.person_ids[idx]],
...
def build_sampler(name, config, phase, datasource, weight, attribute_name, **kwargs):
    ...
    if name == "RandomBalanceBatchSamplerAttribute":
        ...
        return (
            RandomBalanceBatchSamplerAttribute(
        ...
        return (
            RandomBatchSamplerAttributeWeight(
                datasource=datasource,
                weight=weight,
                attribute_name=attribute_name,
                num_attribute=config[phase]["num_attribute"],
