...
if __name__ == "__main__":
    ...
    for f1 in [f.name for f in os.scandir(log_dir) if f.is_dir()]:
        ...
        for i, line in enumerate(info_log):
            ...
            if head[0] == "Model":
                try:
                    saved_info["info"]["Model"] = (
                        head[2].split(" ")[1].split(",")[0].replace(" ", "")
                    )
                except:
                    saved_info["info"]["Model"] = (
                        head[1].split(" ")[1].split(",")[0].replace(" ", "")
                    )
            if head[0] == "Freeze layer":
                saved_info["info"]["Freeze layer"] = 1
            if head[0] == "Loss":
                saved_info["info"]["Loss"] = (
                    head[1].split(" ")[1].split(",")[0].replace(" ", "")
                )
            if head[0] == "Optimizer":
                saved_info["info"]["Optimizer"] = (
                    head[1].split(" ")[1].split(",")[0].replace(" ", "")
                )
            if head[0] == "Lr scheduler":
                saved_info["info"]["Lr scheduler"] = (
                    head[1].split(" ")[1].split(",")[0].replace(" ", "")
                )
            saved = True
        info_log.close()
    ...
    for x in tqdm(all_saved_info):
        ...
        des = os.path.join(
            ...
            x["info"]["Model"],
            x["info"]["Loss"],
            "Freeze layer".replace(" ", "") + "-" + str(x["info"]["Freeze layer"]),
            x["info"]["Optimizer"],
            x["info"]["Lr scheduler"],
            x["id"],
        )
